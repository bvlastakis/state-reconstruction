...
def reg_speedy(M, W, N):
    #Unconstrained regression using matrix inversion
    invM = np.dot( np.linalg.inv( np.dot(M.T, M) ), M.T )
    #self.inverse_matrix = invM
    x = np.dot(invM, W)

    reg = complexUnstack(x).reshape( (N, N) )

    #remove negative eigenvalues a la Smolin (2011)
    evalues, evectors = np.linalg.eigh(reg)
    cumulative = 0
    size = len(evalues)
    ...
    #create rho = Vector_mat * Value_mat * Vector_mat^dagger
    rho = np.dot(np.dot(evectors, np.diag(evalues)), evectors.transpose().conj())
